...
results_dir = PLAYGROUND_DIR/f"seq_mnist_{model_type}_inhib_NC_test"
...
filestr = str(params.get_exp_savepath() / f'{model_type}_learning_curves_seed{seed}_inhib{inhib_ratio}_lr{lr}_GC{max_gn}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/{model_type}_model_test_seed{seed}_inhib{inhib_ratio}_lr{lr}_GC{max_gn}.pth')