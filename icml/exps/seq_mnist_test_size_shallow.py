...
results_dir = PLAYGROUND_DIR/f"seq_mnist_test_{model_type}_shallow_NC"
...
(seed, rad, n_hidden) = para_comb[array_index-1]
#lr = round(lr, 6)
...
#             ]
    ...
    num = (rad)**2
    cells = [EiRNNCell(input_features, n_hidden,max(n_hidden//(9+1),1),max(n_hidden//(9+1),1),nonlinearity=F.relu, i2h_init_policy=EiRNNCell_U_InitPolicy(), h2h_init_policy=EiRNNCell_W_InitPolicy(numerator=num, random=False)),
...
filestr = str(params.get_exp_savepath() / f'{model_type}_learning_curves_seed{seed}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_shallow_test_{model_type}_model_seed{seed}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}.pth')
