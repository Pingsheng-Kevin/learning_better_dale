...
results_dir = PLAYGROUND_DIR/f"seq_mnist_test_{model_type}_hidden_NC"
...
filestr = str(params.get_exp_savepath() / f'{model_type}_learning_curves_seed{seed}_lr{lr}_GC{max_gn}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_test_{model_type}_model_seed{seed}_hidden_lr{lr}_GC{max_gn}.pth')
