...
results_dir = PLAYGROUND_DIR/f"adding_10_{model_type}_neural_comp_test"
...
filestr = str(params.get_exp_savepath() / f'{model_type}_learning_curves_seed{seed}_lr{params.lr}_GC{max_gn}_decay{decay}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/adding_{model_type}_seed{seed}_lr{params.lr}_GC{max_gn}_decay{decay}.pth')
