...
results_dir = PLAYGROUND_DIR/f"ptb_3layers_{model_type}_neural_comp_test"
...
def train(lr, n_epochs, cells, seed, max_gn=None, decay=decay, model_type=model_type):
    ...
    filestr = str(params.get_exp_savepath() / f'{model_type}_learning_curves_seed{seed}_lr{params.lr}_GC{max_gn}')
    np.savez(filestr, **results)
    return results, model

results, model = train(lr, n_epochs, cells, seed, max_gn=max_gn, decay=decay, model_type=model_type)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/ptb_3layers_test_{model_type}_seed{seed}_lr{init_lr}_GC{max_gn}.pth')
