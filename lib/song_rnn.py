...
class ColumnEiCell(BaseRNNCell):
    ...
    def __init__(self, n_input, layer_params: '(n_hidden, ratio)', nonlinearity=None, learn_hidden_init=False,
                 ...
                 clamp=True, W_dropconnect=0.0, U_dropconnect=0.0):
        """
    ...
    def forward(self, x):
        ...
        x: input of shape input_dim x batch_dim
           ...
           W is h x h
        """
        U_drop = F.dropout(self.U, p=self.U_dropconnect) * (1-self.U_dropconnect)
        W_drop = F.dropout(self.W, p=self.W_dropconnect) * (1-self.W_dropconnect)
        # print(self.U.shape, x.shape, self.W.shape, self.h.shape, self.b.shape)
        self.z = torch.mm(U_drop, x.T) + torch.mm(W_drop, self.h) + self.b
