...
class EiRNN_EG():
    '''
    This update does EG (i.e. e**(p.grad * -lr) ).

    It assumes the layer has the parameters of an EiDenseWithShunt layer.

    This should furthest right when composed with mixins for correct MRO.
    '''
    def __init__(self, max_inf_norm=None, renorm=None) -> None:
        if max_inf_norm is not None: assert max_inf_norm > 0
        self.max_inf_norm = max_inf_norm
        self.renorm = renorm
        # print("To refactor!, use (SGD, EiRNN_clamp_mixin) instead")
    ...
    def update(self, layer, **args):
        ...
        with torch.no_grad():
            if self.max_inf_norm is not None:
                layer.b.grad = torch.clamp(layer.b.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Uex.grad = torch.clamp(layer.Uex.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Uei.grad = torch.clamp(layer.Uei.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Uix.grad = torch.clamp(layer.Uix.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Wex.grad = torch.clamp(layer.Wex.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Wei.grad = torch.clamp(layer.Wei.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.Wix.grad = torch.clamp(layer.Wix.grad, min=-self.max_inf_norm, max=self.max_inf_norm)

            layer.b *=  torch.exp(layer.b.grad * -lr)
            layer.Uex *= torch.exp(layer.Uex.grad * -lr)
            layer.Uei *= torch.exp(layer.Uei.grad * -lr)
            layer.Uix *= torch.exp(layer.Uix.grad * -lr)
            layer.Wex *= torch.exp(layer.Wex.grad * -lr)
            layer.Wei *= torch.exp(layer.Wei.grad * -lr)
            layer.Wix *= torch.exp(layer.Wix.grad * -lr)
            if hasattr(layer, 'U_alpha'):
                if self.max_inf_norm is not None: layer.U_alpha.grad = torch.clamp(layer.U_alpha.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.U_alpha *= torch.exp(layer.U_alpha.grad * -lr)
            if hasattr(layer, 'W_alpha'):
                if self.max_inf_norm is not None: layer.W_alpha.grad = torch.clamp(layer.W_alpha.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.W_alpha *= torch.exp(layer.W_alpha.grad * -lr)
            if hasattr(layer, 'U_g'):
                if self.max_inf_norm is not None: layer.U_g.grad = torch.clamp(layer.U_g.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.U_g *= torch.exp(layer.U_g.grad * -lr)
            if hasattr(layer, 'W_g'):
                if self.max_inf_norm is not None: layer.W_g.grad = torch.clamp(layer.W_g.grad, min=-self.max_inf_norm, max=self.max_inf_norm)
                layer.W_g *= torch.exp(layer.W_g.grad * -lr)
...
class EiRNN_cEG_UpdatePolicy(BaseUpdatePolicy, EiRNNcSGD_Mixin, EiRNN_EG):
    def __init__(self, max_inf_norm=None, renorm=None):
        super(EiRNN_cEG_UpdatePolicy, self).__init__()
        self.max_inf_norm=max_inf_norm
        self.renorm=renorm
...
class SGD_Clip_UpdatePolicy(BaseUpdatePolicy, ClipGradNorm_Mixin, SGD):
    def __init__(self, max_grad_norm=None):
